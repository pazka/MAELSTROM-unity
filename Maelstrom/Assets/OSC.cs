...
namespace Maelstrom.Unity
{
    ...
    public class UdpPacketIO
    {
        ...
        public int ReceivePacket(byte[] buffer)
        {
            ...
            var iep = new IPEndPoint(IPAddress.Any, LocalPort);
            var incoming = Receiver.Receive(ref iep);
            var count = Math.Min(buffer.Length, incoming.Length);
    ...
    public class OSC
    {
        ...
        public OSC(int inPort, string outIp, int outPort)
        {
            ...
            EditorApplication.playModeStateChanged += HandleOnPlayModeChanged; //FIX FOR UNITY POST 2017
#endif
        }
        ...
        private void HandleOnPlayModeChanged(PlayModeStateChange state) //FIX FOR UNITY POST 2017
        {
        ...
        {
            var al = (ArrayList)Hashtable.Synchronized(AddressTable)[key];
            if (al == null)
            {
                al = new ArrayList();
                al.Add(ah);
            ...
            if (messagesReceived.Count > 0)
                ...
                lock (ReadThreadLock)
                {
                    foreach (OscMessage om in messagesReceived)
                    {
                        ...
                        var al = (ArrayList)Hashtable.Synchronized(AddressTable)[om.address];
                        if (al != null)
    ...
    */
        ...
        private void Read()
        {
            try
            {
                while (ReaderRunning)
                {
                    var length = OscPacketIO.ReceivePacket(buffer);
        ...
        public void Send(OscMessage oscMessage)
        {
            ...
            var length = OscMessageToPacket(oscMessage, packet, 1000);
        ...
        {
            var packet = new byte[1000];
            var length = OscMessagesToPacket(oms, packet, 1000);
            OscPacketIO.SendPacket(packet, length);
        }
        ...
        public static int OscMessagesToPacket(ArrayList messages, byte[] packet, int length)
        {
            ...
            else
            {
                ...
                foreach (OscMessage oscM in messages)
                {
                    ...
                    index += 4;
                    var packetStart = index;
                    index = OscMessageToPacket(oscM, packet, index, length);
                    var packetSize = index - packetStart;
        ...
        private static int ExtractMessages(ArrayList messages, byte[] packet, int start, int length)
        {
            ...
            switch ((char)packet[start])
            {
                case '/':
                    ...
                    break;
                case '#':
                    var bundleString = ExtractString(packet, start, length);
                    if (bundleString == "#bundle")
                    {
                        ...
                        while (index < length)
                        {
                            var messageSize = (packet[index++] << 24) + (packet[index++] << 16) +
                                              (packet[index++] << 8) +
                                              packet[index++];
                            /*int newIndex = */
        ...
        private static int ExtractMessage(ArrayList messages, byte[] packet, int start, int length)
        {
            ...
            var index = start + PadSize(oscM.address.Length + 1);
            var typeTag = ExtractString(packet, index, length);
            index += PadSize(typeTag.Length + 1);
            //oscM.values.Add(typeTag);
            foreach (var c in typeTag)
                switch (c)
                {
                    case ',':
                        break;
